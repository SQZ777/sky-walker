...
def test_callback_receipt_after_capture_stop_is_invalid(tmp_path):
    records = _record_fixtures()
    later = dict(records[-1])
    later["callback_sequence"] = 11
    later["receipt_timestamp"] = "2026-08-18T00:00:22Z"
    later["location_timestamp"] = "2026-08-18T00:00:22Z"
    records.append(later)
    jsonl_path = tmp_path / "post-stop-callback.jsonl"
    jsonl_path.write_text(
        "\n".join(json.dumps(record) for record in records), encoding="utf-8"
    )

    with pytest.raises(ProbeInputError, match="outside the capture window"):
        validate_files(FIXTURES / "pass.manifest.json", jsonl_path)
