...
def test_source_probe_can_label_sky_walker_ble_lns_evidence():
    models = (PROBE / "SourceProbe" / "ProbeModels.swift").read_text(
        encoding="utf-8"
    )

    assert 'case skyWalkerBleLNS = "sky-walker-ble-lns"' in models
    assert 'case .skyWalkerBleLNS: return "Sky Walker BLE LNS"' in models
